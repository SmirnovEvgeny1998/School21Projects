...
1. При помощи асинхронного программирования имитировать сражение Нео и агентов.
2. Создать сервер и клиент используя FastAPI и aiohttp или httpx ,которые будут отправлять HTTP GET запросы для выбранных клиентом страниц и возвращать их значения.
3. Сервер должен кешировать запрошенные ранее страницы и вести счет запрошенных доменов при помощи Redis.
...
`server.py`. It is recommended to use aiohttp or httpx for the client
side, and FastAPI for the server side. All the I/O code should be
asynchronous.
...
- client submits all the URLs via HTTP POST request as a JSON list to a server endpoint 
  `/api/v1/tasks/`
- server responds with HTTP 201 created and a task object (consider using PyDantic)
- task object includes a status "running" and an ID, which is UUID4
...
In synchronous world people often implement this using modules like Celery, but
in this task no external workers are required, so all the server should be in a single Python file,
and all the code should use async/await paradigm.
...
asynchronous and use async/await paradigm. You may consider using aioredis library
...
Python, Redis, FastAPI, aiohttp, httpx.
